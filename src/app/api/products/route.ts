...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(searchParams, "ini dari route product");
    // if there is id param, get product by id with category, reviews, and related products
    if (id) {
    ...
    if (search) {
      conditions.push(
        //atau produkj name atau description yang mengandung search
        or(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await db.delete(product).where(eq(product.id, productId)).returning();
